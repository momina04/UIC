...
/* Retrunrs struct timeval struct after adding given msecs to current time */
	...
	while(1) {
		...
		if(head!=tail && ms_from_now(packets[head].tx_at)<=0) {		 
			...
			if(random() % 1000 < forward_rate*1000) {
				...
				printf("<RITESH> Sent packet %s:%d\n",inet_ntoa(addr.sin_addr),packets[head].out_port);
			}
			else {
				printf("<RITESH> Forwarding probability dropped.\n");
