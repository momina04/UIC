...
void send_ack(int sock, int seq_no)
{
	...
	send(sock, packet, sizeof(struct hw5_hdr), 0);
	sequence_number++;
}
...
int receive_ack(int sock, int timeout)
{
    int status = 1;
    char buf[1400];
	struct hw5_hdr *hdr = (struct hw5_hdr*)buf;
...
void rel_send(int sock, void *buf, int len)
{
    ...
    int ack_time;
    if (user == UNKNOWN){
        fprintf(stderr, "<RITESH> Identified sender\n");
        user = SENDER;
        fp = fopen("values","w+");
        if(fp == NULL){
	...
	fprintf(stderr,"<RITESH> Sent packet - %d\n",ntohl(hdr->sequence_number));
    ...
    while(status < 0){
        if(this_is_close_packet){
            ...
            if(close_retry_attempts == 20){
                fprintf(stderr,"<RITESH> Close Packet -  %d was not acknowledged - %d times\n",ntohl(hdr->sequence_number),close_retry_attempts);
                break;
            }
        }
        ...
        if(len >= 0){ /*IMP: need to wait for ACK for last packet/close packet, this is to inform other end to return 0 on rel_recv() */
            fprintf(stderr,"<RITESH> Waiting for ACK %d ms <<\n",rtt );
            status = receive_ack(sock, rtt);
            if(status < sequence_number && status > 0){
                fprintf(stderr,"<RITESH> Recevied Previous ACK - %d..ignoring...resending - %d\n",status, ntohl(hdr->sequence_number));
                status = -1;
                continue;
            }
            else if(status > sequence_number){
                fprintf(stderr,"<RITESH> <FATAL> How is this possible");
                exit(2);
            }
            if(status > 0){
                ...
                fprintf(stderr,"<RITESH> GOT Ack - %d\n", status);
            }
            else{
                fprintf(stderr,"<RITESH> MISSED Ack....Retransmitting - %d\n",ntohl(hdr->sequence_number));
...
int rel_recv(int sock, void * buffer, size_t length) {
    ...
    if (user == UNKNOWN){
        fprintf(stderr, "<RITESH> Identified receiver\n");
        user = RECEIVER;
    }

again:
	fprintf(stderr,"<RITESH> Waiting for packet\n");
	...
	fprintf(stderr,"<RITESH> Got packet\n");
	if(ntohl(hdr->sequence_number) <= last_seq_no){
        send_ack(sock,ntohl(hdr->sequence_number));
        fprintf(stderr,"<RITESH> Detected Retramsitt - %d ..sending ack again and discarding\n", ntohl(hdr->sequence_number));
        goto again;
     }
	...
	fprintf(stderr,"<RITESH> Sending ACK - %d\n", ntohl(hdr->sequence_number));
	char buf[100];
	...
	//send(sock, "ACK", 3, 0);
	//syncfs(sock);
	fprintf(stderr,"<RITESH> ACK sent\n",ntohl(hdr->sequence_number));
