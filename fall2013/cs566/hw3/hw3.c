...
double A[N][N];
double AA[N][N];   /* to hold result of A*A */
...
int main(int argc, char** argv)
{
  ...
  if (rank==0)  // root/coordinator process
    ...
    MPI_Waitany(size,ireq,&j,&stat); // join on sends
      ...
      for(i=1;i<size;i++)
    MPI_Irecv(AA+(i*interval),interval*N,MPI_DOUBLE,i,i,
          MPI_COMM_WORLD,ireq+i);
      for(i=1;i<size;i++)
      {
  ...
  else          // worker process
    {
      ...
      MPI_Send(AA+(rank*interval),interval*N,MPI_DOUBLE,0,rank,
        MPI_COMM_WORLD);
    }
  MPI_Finalize();
} //main
...
void compinterval(int start, int interval)
{
  ...
  for (i=start;i<start+interval;i++)
    for(j=0;j<N;j++)
      {
    AA[i][j]=0;
    for(k=0;k<N;k++) AA[i][j] += A[i][k] * B[k][j];
      }
}
